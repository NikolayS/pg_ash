...
# 3. Installs pg_ash 2.0 (from the mounted /repo/sql/ash-install.sql).
...
if ! $PSQL -d postgres -tc "select 1 from pg_database where datname='demo'" | grep -q 1; then
  ...
  # Install the current 2.0 installer. Keep in sync with
  # `devel/scripts/ash_sql_chain.py fresh-install-path` if that path moves.
  echo "[entry] installing pg_ash 2.0 (\i /repo/sql/ash-install.sql)"
