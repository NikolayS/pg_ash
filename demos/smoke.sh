...
actual_version="$(
  "${psql_demo[@]}" \
    --command="select version from ash.config where singleton"
)"
...
cron_available="$(
  "${psql_demo[@]}" \
    --command="select value from ash.status() where metric = 'pg_cron_available'"
)"
[[ "$cron_available" == "yes" ]] \
...
sampler_jobs="$(
  "${psql_demo[@]}" \
    --command="select count(*) from cron.job where jobname = 'ash_sampler'"
)"
[[ "$sampler_jobs" == "1" ]] \
...
while :; do
  sample_count="$(
    "${psql_demo[@]}" --command="select count(*) from ash.sample"
  )"
  (( sample_count > 0 )) && break
  (( SECONDS < sample_deadline )) \
