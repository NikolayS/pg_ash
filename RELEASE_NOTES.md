...
- A schema owner with `pg_monitor` can start libpq-backed pg_cron jobs without
  ...
  when that permission is absent. Inactive sampler/rotation jobs recreated
  through the fallback without `alter_job` receive the same permitted socket
  adjustment as new jobs; reactivation through `alter_job` preserves their
  connection settings. Minute scheduling tolerates pg_cron versions with named
  jobs but no optional `alter_job` API. (#260)
...
- **Reader and input-validation edge cases fixed.** `top_waits()` now treats `p_limit` as a hard output row limit including `Other`, zero timeline buckets raise explicit validation errors, `ash.start(NULL)` returns a clear error row, and `ash.grant_reader()` no longer grants EXECUTE on internal mutating helper `_register_wait()` (issue #91; [PR #99](https://github.com/NikolayS/pg_ash/pull/99)). `ash.start()` also rejects fractional-second and oversized intervals without rounding or integer-overflow errors (issue #141; [PR #142](https://github.com/NikolayS/pg_ash/pull/142)).
