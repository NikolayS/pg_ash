...
--   - Reader UX (#37):
--       * ash._active_slots() now takes optional p_interval and raises a
--         per-transaction NOTICE when the requested interval exceeds
--         2 * rotation_period. All reader call sites updated to pass
--         p_interval through.
--       * Removed dead helper ash._validate_data().
--       * BREAKING: ash.samples_by_database() column total_backends
--         renamed to total_backend_samples (requires drop + re-create).
...
-- Redefine _active_slots with optional p_interval + NOTICE on oversized
-- intervals. Signature changes (arity), so drop the old 0-arg version first.
drop function if exists ash._active_slots();
...
-- Reader: top_waits — pass p_interval into _active_slots
...
-- Reader: samples_by_database — BREAKING column rename requires drop first
-- (create or replace cannot change return type signature).
...
-- Reader: event_queries
drop function if exists ash.event_queries(text, interval, int);
drop function if exists ash.event_queries(text, interval, int, int, boolean);
