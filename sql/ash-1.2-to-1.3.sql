...
-- Changes:
--   - Azure / managed-service compatibility for cron.job operations
--   - ash.debug_logging() — per-session RAISE LOG in take_sample()
...
-- Security: restrict admin functions to schema owner
do $$
declare
  v_owner text;
begin
  select nspowner::regrole::text into v_owner from pg_namespace where nspname = 'ash';
