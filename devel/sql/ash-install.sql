...
begin
  ...
  if v_seconds_exact > 43200 then
    job_type := 'error';
    job_id := null;
    status := format(
      'interval exceeds maximum supported cadence of 12 hours (43200s), '
      'got %s. Use one of the documented divisor cadences.', every);
    return next;
    return;
  end if;
  ...
  if v_seconds <= 59 then
    ...
    -- Convert to cron only when the step divides the containing hour evenly.
    ...
    if 60 % (v_seconds / 60) <> 0 then
      job_type := 'error';
      job_id := null;
      status := format(
        'minute interval must divide one hour evenly; supported minute '
        'counts are 1, 2, 3, 4, 5, 6, 10, 12, 15, 20, and 30, got %s',
        every);
      return next;
      return;
    end if;
    ...
    -- Convert to cron only when the step divides the containing day evenly.
    if v_seconds % 3600 <> 0 then
      job_type := 'error';
      job_id := null;
      status := format(
        'interval must be exact hours (3600s, 7200s, etc., up to 12h), '
        'got %s', every);
      return next;
      return;
    end if;
    v_hours := v_seconds / 3600;
    if 24 % v_hours <> 0 then
      job_type := 'error';
      job_id := null;
      status := format(
        'hour interval must divide one day evenly; supported hour counts '
        'are 1, 2, 3, 4, 6, 8, and 12, got %s',
        every);
      return next;
      return;
    end if;
  ...
  if not v_pg_cron_available then
    ...
    job_id := null;
    status := format(
      'pg_cron unavailable — configure an external scheduler to execute '
      'SELECT ash.take_sample() exactly every %s; using another cadence '
      'makes AAS values inaccurate',
      every);
    return next;
    ...
    job_id := null;
    status :=
      'call ash.rotate() daily; it skips until at least 90% of '
    ...
    raise notice
      'pg_cron is unavailable. Configure an external scheduler to execute '
      'SELECT ash.take_sample() exactly every %; using another cadence '
      'makes AAS values inaccurate.',
      every;
    v_cron_database :=
    ...
    raise notice
      '  system scheduler: execute "SELECT ash.take_sample()" exactly every %',
      every;
    raise notice
      '  use a scheduler that anchors invocations to wall-clock deadlines; '
      'psql \watch and sleep-after-work loops drift by execution time';
    raise notice
      'Call ash.rotate() daily; it skips until at least 90%% of '
      'rotation_period has elapsed.';
    raise notice
  ...
  /*
   * Warn about pg_cron run history overhead. pg_cron adds one
   * cron.job_run_details row per run and has no built-in purge; cron.log_run
   * can disable that history entirely.
   */
  begin
    if current_setting('cron.log_run', true)::bool then
      raise notice
        'hint: sub-minute pg_cron sampling can add one '
        'cron.job_run_details row per sampler run and has no built-in '
        'retention.';
      raise notice
        'to disable: alter system set cron.log_run = off; then restart '
        'PostgreSQL.';
      raise notice
  ...
  if lookback is not null and lookback > v_raw_retention then
    ...
    if v_already is null or v_already = '' then
      raise notice
        ...
        'Shorten the interval. To retain more raw data, first increase '
        'rollup_1m_retention_days as needed to satisfy '
        '(num_partitions - 1) * rotation_period, then increase '
        'rotation_period or rebuild with more partitions.',
  ...
  if since is not null and until is not null
    ...
    if v_already is null or v_already = '' then
      raise notice
        ...
        'partition(s) plus the current partial partition are retained. '
        'Adjust the range. To retain more raw data, first increase '
        'rollup_1m_retention_days as needed to satisfy '
        '(num_partitions - 1) * rotation_period, then increase '
        'rotation_period or rebuild with more partitions.',
...
begin
  ...
  metric := 'sampling_config_enabled';
  value := v_config.sampling_enabled::text;
  return next;
  ...
  metric := 'interrupted_sample_calls';
  value := v_config.missed_samples::text;
  return next;
  metric := 'sampling_evidence';
  value := 'no heartbeat is stored; idle ticks and scheduler outages '
  ...
  while v_minute_start < v_now_minute_ts and v_batch_remaining > 0 loop
    ...
    if v_count = 0 then
      ...
      if v_has_later_data then
        raise notice
          'ash.rollup_minute: minute % has no stored activity observation '
          'but later data exists; this may be an idle minute, a missed '
          'sampler tick, or data lost before rollup.',
...
begin
  ...
  if not v_cov1 then
    raise notice
      'ash.compare: window 1 (% to %) has no stored activity observations '
      '— its columns and avg_delta are NULL, not zero. pg_ash cannot '
      'distinguish idle time from missed sampling; check retention and '
      'scheduler health before reading a delta.',
      v_aas1.period_start, v_aas1.period_end;
  end if;
  if not v_cov2 then
    raise notice
      'ash.compare: window 2 (% to %) has no stored activity observations '
      '— its columns and avg_delta are NULL, not zero. pg_ash cannot '
      'distinguish idle time from missed sampling; check retention and '
      'scheduler health before reading a delta.',
...
begin
  ...
  if v_n = 0 then
    ...
    if v_available_source is not null then
      raise notice
        'ash.report: no rollup_1m activity rows for [%, %); % has stored '
        'activity rows, but report reads rollup_1m for base metrics and '
        'returns NULL',
        ash.ts_to_timestamptz(v_start_ts),
        ash.ts_to_timestamptz(v_end_ts),
        v_available_source;
...
begin
  ...
  if v_aas.buckets_with_data = 0 then
    return query select
      ...
      '(idle time and sampler gaps are indistinguishable)'::text;
    return;
  end if;
...
$$START HERE (US-1 triage): AAS for six standard trailing windows (1m, 5m, 1h, 1d, 1w, 1mo; 1mo is a fixed 30 days) requested to end at until (default now()), one row each. Columns (period, period_start, period_end, source, bucket, buckets_with_data, avg_aas, peak_aas, p99_aas): period bounds are effective and may snap outward for hour-only reads. With a fixed sampling interval and independently confirmed scheduler health, peak materially above average suggests a sampled short spike; it does not prove one. buckets_with_data counts stored activity-bearing buckets at the effective grain named by bucket, not successful sampler ticks; idle time, outages, retention loss, and delayed rollup are indistinguishable when no row is stored (#137). Genuine rollup_1h.minute_counts keeps minute grain; legacy/incomplete detail on a minute-capable plan reports source = rollup_1h_flat, bucket = 1 hour, and NULL sub-hour peak/p99 instead of fabricated minute extremes. Next: locate the spike in time with ash.timeline(), then drill with ash.top().$$;

comment on function ash.aas(timestamptz, timestamptz, text, text, bigint, name, interval) is
$$Scalar AAS summary for one requested window [since, until) (defaults: last 1 hour). Optional uniform filters wait_event_type/wait_event/query_id/database. Columns (period_start, period_end, source, effective_bucket, buckets_expected, buckets_with_data, avg_aas, peak_aas, p99_aas, backend_seconds). period_start/period_end are effective bounds: hour-only rollup_1h reads snap partial bounds outward. avg_aas and backend_seconds are arithmetic over retained activity-bearing observations at the current configured sample_interval; they do not prove historical cadence or sampler coverage (#137). buckets_with_data is likewise not a successful-tick count. peak/p99 are NULL when retained grain exceeds the requested bucket, never hour averages presented as minute extremes. Genuine minute_counts preserves minute grain for unfiltered/database-only reads; legacy/incomplete detail on that minute-capable plan reports source = rollup_1h_flat. Every explicit query_id filter forces raw; it raises only when ordinary planning selects a rollup with stored activity in the requested pre-raw slice, while a young query filter reads available raw rows when the first rollup contains only raw's first retained minute. source = raw|rollup_1m|rollup_1h|rollup_1h_flat|none. Next: ash.top('query_id', wait_event => ...).$$;
...
$$Before/after comparison of two windows (US-7). dimension => null gives one overall row; a dimension gives top keys by abs(avg_delta). Columns (key, query_text, source_1, source_2, period_start_1, period_end_1, period_start_2, period_end_2, effective_bucket_1, effective_bucket_2, avg_aas_1, avg_aas_2, avg_delta, peak_aas_1, peak_aas_2, p99_aas_1, p99_aas_2, pct_1, pct_2). Sources and effective plans are disclosed per window. If retained read grains differ, both peak values and both p99 values are NULL as incomparable even when explicit effective buckets match; arithmetic over retained observations remains comparable. Sides with no stored activity observations and avg_delta are NULL plus a NOTICE; within a side that has stored activity, an absent key is represented as zero relative to that recorded activity, not as proof of true inactivity. Explicit query_id filters force raw on both sides; unfiltered rollup query comparisons include and NULL-safe-pair each side's compacted-attribution residual.$$;

comment on function ash.samples(timestamptz, timestamptz, int, text, text, bigint, name) is
$$Decoded raw sample rows, newest first (US-5 raw evidence) over [since, until) (default last 1 hour), up to n. Uniform filters wait_event_type/wait_event/query_id/database. Columns (sample_time, database_name, active_backends, wait_event, query_id, query_text). query_text needs pg_stat_statements. A caller can see text for statements owned by its own role; pg_read_all_stats (for example through pg_monitor membership) is additionally required for cross-role statement text. A plain ash.grant_reader() role therefore sees NULL for other roles' text even with pgss installed. When pgss lives outside public, the caller also needs USAGE on that schema, else query_text is null. Reads ash.sample directly (raw retention only).$$;

comment on function ash.report(timestamptz, timestamptz, int, int) is
$$Machine-readable load report as one jsonb (US-8) for [since, until) (no bounds: last 1 day; until-only: preceding 1 hour). Per selected class (cpu=CPU*, io=IO, ipc=IPC, lock=Lock, lwlock=LWLock; total is the sum of those five classes, not all pg_ash activity) at 1-minute resolution: aas_avg / aas_worst1m / aas_p99 / aas_p999. Other recorded wait-event types remain queryable through ash.top('wait_event_type'); exclusion from this payload does not prove they are harmless. Plus top_events_{worst1m,p99,p999} (keys io/ipc/lock/lwlock, entries "event(aas)") and top_queryids_{worst1m,p99,p999} (keys total+the four non-cpu classes, entries "queryid(aas)"). Base metrics and top events read ash.rollup_1m. Query attribution additionally uses raw samples per extreme minute: a top_queryids key appears when raw samples still cover that class's worst/percentile minute(s), even if the window start predates raw retention (percentile sets attribute over their raw-covered minutes). top_queryids_available (boolean, always present) says whether this call produced any attribution — branch on it, not on key absence. coverage {from, to, source, minutes_expected, minutes_with_data, raw_retention_start} aligns the payload window with general readers; minutes_with_data counts stored activity-bearing rollup minutes, not sampler heartbeats, and does not establish sampler completeness or numeric equivalence. raw_retention_start is the logical planning/loss boundary, not the physical query-attribution cutoff. vcpus (echoed, never used) and cluster_name are pass-throughs. If rollup_1m has no stored activity rows for the window, returns null even when raw or rollup_1h has rows, emits a NOTICE naming that source, and never raises. Payload contract is frozen per 2.0 minor line (keys only added, never renamed/removed); scoring/normalization is the consumer's job.$$;
...
$$Installation health snapshot (readable by monitoring roles): configuration and sampling-enabled state; sample interval and the day-granular rotation_period contract (whole days, minimum 1 day); current partition slot; raw/rollup row counts and boundaries; dictionary utilization; failure/error counters; pg_cron availability and visible job metadata; and version. last_activity_sample_ts is the newest activity-bearing sample row, not the last attempted sampler tick. pg_ash stores no sampler heartbeat, so idle periods and scheduler outages are indistinguishable. raw_retention_start is a reusable minute-aligned planning/loss boundary; rollup_1m_retention_start and rollup_1h_retention_start are physical starts. Returns (metric, value) rows. Start here when pg_ash misbehaves; readers are documented on the schema: obj_description('ash'::regnamespace).$$;

comment on function ash.start(interval) is
$$Admin: start sampling — schedules take_sample() (every => ..., default 1 second) plus rollup_minute()/rollup_hour()/rollup_cleanup() and a daily rotation check via pg_cron when available. Accepted cadences are 1-59 whole seconds; whole-minute counts that divide 60 (1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30); or whole-hour counts that divide 24 (1, 2, 3, 4, 6, 8, 12). Divisor-only minute/hour schedules prevent calendar-boundary gaps from disagreeing with AAS weighting. rotation_period accepts whole days only (minimum 1 day); daily checks skip until at least 90% of that period has elapsed. Without pg_cron, start enables sampling and prints that an external scheduler must execute take_sample() exactly at every; another cadence makes AAS inaccurate. Idempotent. Inverse: ash.stop(). Check with ash.status().$$;
...
$$Admin/internal: take one activity/wait-event sample of pg_stat_activity into the current slot using the configured interval. Returns the number of per-database sample rows inserted; returns 0 when sampling is disabled/locked or no activity row is written, and -1 when interrupted (interrupted_sample_calls in ash.status() increments). Call manually only for testing; continuous sampling belongs to ash.start() or an exact-cadence external scheduler.$$;

comment on function ash.rotate() is
$$Admin: rotate to the next partition slot, rolling up and truncating the oldest (checked daily by ash.start()). It becomes eligible after at least 90% of rotation_period has elapsed; earlier calls return skipped. Readable raw retention is approximately (num_partitions - 2) * rotation_period. The current partial period may add more. The pre-truncation guard requires minute rollups only while they remain inside rollup_1m_retention_days. Failed attempts increment consecutive_rotate_failures in ash.status(); successful rotation resets it. Safe to call manually.$$;
...
$$Admin: fold completed hours of ash.rollup_1m into ash.rollup_1h. minute_counts preserves available per-minute totals and must sum to the hourly wait total; missing stored rows remain NULL slots because idle and uncovered minutes are indistinguishable (#137). Returns completed hour grains processed and watermark-advanced; empty hours count even when they write no ash.rollup_1h row, while an already-caught-up call returns 0.$$;
...
begin
  ...
  if not pg_catalog.pg_has_role(role, 'pg_read_all_stats', 'MEMBER')
    ...
    raise notice
      ...
      'member); query_text for statements owned by other roles will be NULL '
      '— grant pg_read_all_stats directly (or pg_monitor) to % if cross-role '
...
comment on function ash.grant_reader(name) is
  'Grants the supported complete reader bundle (USAGE on schema ash, EXECUTE on all reader functions AND the internal helpers they depend on, SELECT on reader tables incl. partitions) to a monitoring role. Granting table access or EXECUTE on an individual reader alone is insufficient because readers are SECURITY INVOKER and call shared internal helpers (also revoked from PUBLIC). Idempotent. Inverse: ash.revoke_reader(name). Complete reader bundles are preserved automatically by ash.rebuild_partitions(N, ''yes'').';
