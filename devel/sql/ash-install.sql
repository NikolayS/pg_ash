...
create table if not exists ash.config (
  ...
  version                    text not null default '2.0-rc1',
 ...
 */
update ash.config set version = '2.0-rc1' where singleton;
alter table ash.config alter column version set default '2.0-rc1';
...
declare
  ...
  v_can_alter_job boolean := false;
  v_job_was_inactive boolean := false;
...
begin
  ...
  /*
   * pg_cron 1.3 has named scheduling but no alter_job. Probe its optional
   * signature as an OID so an absent function yields NULL, not an error.
   */
  v_can_alter_job := coalesce(pg_catalog.has_function_privilege(
  ...
  if v_sampler_job is not null then
    ...
    if v_canonical_command is null then
      if v_can_alter_job then
        perform cron.alter_job(
          job_id := v_sampler_job, schedule := v_schedule, active := true
        );
      else
        ...
        v_job_was_inactive := not (
          select active from cron.job where jobid = v_sampler_job
        );
        if v_job_was_inactive then
          begin
        ...
        if v_job_was_inactive and not v_skip_nodename_update then
  ...
  if v_rotation_job is not null then
    ...
    if v_canonical_command is null then
      if v_can_alter_job then
        ...
        -- job so explicit start can reactivate it without privileged grants.
        v_job_was_inactive := not (
          select active from cron.job where jobid = v_rotation_job
        );
        if v_job_was_inactive then
          begin
  ...
  begin
    if current_setting('cron.log_run', true)::bool then
      ...
      raise notice
        ...
        'then restart Postgres (cron.log_run is restart-only).';
      raise notice
...
begin
  ...
  -- Floor before rounding, then align again after the int4 horizon clamp.
  v_start_ts := (ash.ts_from_timestamptz(date_trunc('minute', v_from)) / 60) * 60;
  v_end_ts := (ash.ts_from_timestamptz(date_trunc('minute', v_to)) / 60) * 60;
  /*
...
begin
   ...
   */
  v_requested_bucket_secs := extract(epoch from bucket);
  v_start1 := (ash.ts_from_timestamptz(date_trunc('minute', v_from1)) / 60) * 60;
  v_end1 := (ash.ts_from_timestamptz(date_trunc('minute', v_to1)) / 60) * 60;
  if v_end1 <= v_start1 then
    v_end1 := least(v_start1::bigint + 60, 2147483647)::int4;
  end if;
  v_start2 := (ash.ts_from_timestamptz(date_trunc('minute', v_from2)) / 60) * 60;
  v_end2 := (ash.ts_from_timestamptz(date_trunc('minute', v_to2)) / 60) * 60;
  ...
  if v_n = 0 then
    ...
    if v_available_source is not null then
      raise notice
        ...
        'but report base metrics require rollup_1m and return NULL',
        ash.ts_to_timestamptz(v_start_ts),
        ash.ts_to_timestamptz(v_end_ts),
        v_available_source;
    end if;
...
$$Machine-readable load report as one jsonb (US-8) for effective minute-aligned [from, to) bounds (no bounds: last 1 day; until-only: preceding 1 hour). Inverted requested bounds raise an error. Otherwise both endpoints are floored to the minute; equal or ordered same-minute bounds expand to one minute, capped at the integer timestamp horizon. coverage.from/to expose the effective interval. Per wait class (cpu=CPU*, io=IO, ipc=IPC, lock=Lock, lwlock=LWLock; total = their sum) at 1-minute resolution: aas_avg / aas_worst1m / aas_p99 / aas_p999. Plus top_events_{worst1m,p99,p999} (keys io/ipc/lock/lwlock, entries "event(aas)") and top_queryids_{worst1m,p99,p999} (keys total+the four non-cpu classes, entries "queryid(aas)"). Query attribution is decided per extreme minute: a top_queryids key appears when raw samples still cover that class's worst/percentile minute(s), even if the window start predates raw retention (percentile sets attribute over their raw-covered minutes). top_queryids_available (boolean, always present) says whether any attribution was possible — branch on it, not on key absence. coverage {from, to, source, minutes_expected, minutes_with_data, raw_retention_start} reconciles the payload against ash.aas()/ash.top() and flags degraded resolution; raw_retention_start is the logical planning/loss boundary, not the physical query-attribution cutoff. vcpus (echoed, never used) and cluster_name are pass-throughs. Base metrics and top events read ash.rollup_1m; top query IDs additionally read raw samples for eligible extreme minutes. If it has no coverage for the window, returns null even when raw or rollup_1h has coverage, emits a NOTICE naming that source, and never raises for missing coverage. Payload contract is frozen per 2.0 minor line (keys only added, never renamed/removed); scoring/normalization is the consumer's job.$$;
...
$$Admin/internal: take one wait-event sample of pg_stat_activity into the current slot (the every-second worker that ash.start() schedules). Returns the number of per-database sample rows inserted, not the number of captured backends; zero can mean no sample was stored. A handled cancellation returns -1 and records a missed sample. Normally scheduled by ash.start(); without pg_cron, use an external scheduler at exactly the configured sample_interval after enabling sampling with ash.start().$$;
