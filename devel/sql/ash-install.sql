 ...
 * Every failure raises from this single statement and rolls back its config,
 * pg_cron, privilege, and DDL changes. A successful rebuild deliberately
 * leaves sampling disabled; call ash.start() explicitly after verification.
 */
...
begin
  ...
  if v_sampler_job is not null then
    ...
    job_id := v_sampler_job;
    status := format(
      'already exists — schedule updated to %s, command repaired, activated',
      v_schedule
    );
    return next;
  else
  ...
  if v_rotation_job is not null then
    ...
    job_id := v_rotation_job;
    status :=
      'already exists — schedule/command reset, job activated';
    return next;
  else
  ...
  /*
   * Schedule rollup cron jobs. Replace an existing job by exact jobid, but
   * treat a genuinely absent row as the normal fresh-start case. Every other
   * unschedule failure is lifecycle failure: name the affected job, preserve
   * its SQLSTATE, and abort so the enclosing ash.start() statement rolls back
   * every earlier job/config change.
   */
...
/*
 * Stop sampling atomically: remove every locally managed pg_cron job and
 * disable sampling. A missing job is already stopped and remains idempotent.
 * Any failure removing an existing row aborts the statement, so the config
 * update and every earlier successful unschedule roll back together.
 */
...
$$Admin: start sampling — schedules take_sample() at the given interval (every => ..., default 1 second; accepts 1–59 whole seconds, whole minutes, or whole hours up to 23h) plus rollup_minute()/rollup_hour()/rollup_cleanup() and a daily rotation check via pg_cron when available. Existing sampler/rotation jobs have their schedule, command, target database, and active state repaired. rotation_period accepts whole days only (minimum 1 day); early daily checks skip until it is due. Without pg_cron, start enables sampling and prints the jobs to schedule externally. Must be called as the ash schema owner. Scheduling/replacement failures raise and roll back every job/config change from the call; absent jobs are created normally. Idempotent. Inverse: ash.stop(). Check with ash.status().$$;

comment on function ash.stop() is
$$Admin: stop sampling — atomically unschedules the pg_cron jobs created by ash.start() and disables sampling (or only disables sampling when pg_cron is absent). Must be called as the ash schema owner; visible managed jobs owned by another role and targeting this database make the call fail closed. Returns one removed row with its real job ID for each job actually unscheduled; absent jobs are idempotent. If any existing job cannot be unscheduled, the error propagates and rolls back every job/config change from the call; ordinary unschedule errors are wrapped to name the job. Collected data is kept and remains readable. Inverse: ash.start().$$;
