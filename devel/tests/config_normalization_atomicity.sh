...
readonly REPO_ROOT
INSTALLER_PATH="${PG_ASH_TEST_INSTALLER:-${REPO_ROOT}/$(python3 "${REPO_ROOT}/devel/scripts/ash_sql_chain.py" fresh-install-path)}"
MIGRATION_PATH="${PG_ASH_TEST_MIGRATION:-${REPO_ROOT}/devel/sql/ash-1.5-to-2.0.sql}"
if [[ ! -f "${MIGRATION_PATH}" ]]; then
  MIGRATION_PATH="${REPO_ROOT}/sql/ash-1.5-to-2.0.sql"
fi
readonly INSTALLER_PATH MIGRATION_PATH
