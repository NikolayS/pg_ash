...
|---|---|---|---|---|
| CPU* | $\color{#50FA7B}{\rule{40pt}{12pt}}$ green | `#50FA7B` | 80, 250, 123 | `\033[38;2;080;250;123m` |
| IdleTx | $\color{#F1FA8C}{\rule{40pt}{12pt}}$ light yellow | `#F1FA8C` | 241, 250, 140 | `\033[38;2;241;250;140m` |
| IO | $\color{#1E64FF}{\rule{40pt}{12pt}}$ vivid blue | `#1E64FF` | 30, 100, 255 | `\033[38;2;030;100;255m` |
| Lock | $\color{#FF5555}{\rule{40pt}{12pt}}$ red | `#FF5555` | 255, 85, 85 | `\033[38;2;255;085;085m` |
| LWLock | $\color{#FF79C6}{\rule{40pt}{12pt}}$ pink | `#FF79C6` | 255, 121, 198 | `\033[38;2;255;121;198m` |
| IPC | $\color{#00C8FF}{\rule{40pt}{12pt}}$ cyan | `#00C8FF` | 0, 200, 255 | `\033[38;2;000;200;255m` |
| Client | $\color{#FFDC64}{\rule{40pt}{12pt}}$ yellow | `#FFDC64` | 255, 220, 100 | `\033[38;2;255;220;100m` |
| Timeout | $\color{#FFA500}{\rule{40pt}{12pt}}$ orange | `#FFA500` | 255, 165, 0 | `\033[38;2;255;165;000m` |
| BufferPin | $\color{#00D2B4}{\rule{40pt}{12pt}}$ teal | `#00D2B4` | 0, 210, 180 | `\033[38;2;000;210;180m` |
...
- CPU\* is green—the conventional color for sampled on-CPU or otherwise
  uninstrumented active work. CPU\* does not prove that a backend was executing
  on a CPU.
...
In `ash.chart()`, ranks 1–3 use distinct block characters, ranks 4+ share `▒`,
and Other uses `·`, so the breakdown remains visible without color:
...
Colors come from the `STABLE` function
`ash._wait_color(event text, color boolean)`. An explicit `color => false` can
still be overridden by the session setting `ash.color`. Disabling color avoids
ANSI escape output, but no zero-overhead guarantee is made.
...
Color assignments vary across tools. Most fixed semantic palettes use green for CPU.
AWS Performance Insights assigns colors dynamically and therefore does not guarantee
that mapping. All hex codes below are from source code or pixel-sampled from published
screenshots.
...
The screenshots sampled here show CPU in green (`#2CA02C`), but dynamic assignment
does not guarantee that mapping. Hex codes pixel-sampled from
...
- Most fixed semantic palettes use green for CPU. AWS Performance Insights assigns
  colors dynamically and therefore does not guarantee that mapping.
