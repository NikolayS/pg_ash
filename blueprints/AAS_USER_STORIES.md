...
- **Time addressing convention (2.0).** Every reader takes `since timestamptz default null` and `until timestamptz default null`, resolved from the window END: `until` → `now()`, then `since` → `until - '1 hour'`; `since > until` raises. The v1.x `f(p_interval)` / `f_at(p_start, p_end)` twin convention is dropped — 2.0 breaks compat, and the single convention halves the surface. ([AAS_API.md §1](AAS_API.md))
